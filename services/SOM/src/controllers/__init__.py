...
def train(*args, **kwargs):
    ...
    if body.get('weights_init'):
        model_params['model__weights_init'] = body.get('weights_init')
    if body.get('unsup_num_iters'):
        model_params['model__unsup_num_iters'] = body.get('unsup_num_iters')

    features = np.array(features).T
    target = np.array(target)
    try:
        result_ml = helper.model_train(model_id, features, target, **model_params)
